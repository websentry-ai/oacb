#!/usr/bin/env bash
#
# oacb-prompt-guard.sh — OACB UserPromptSubmit hook (Codex CLI agent)
#
...
#
# Codex-specific additions to OACB-PROMPT-004:
#   --yolo                              (Codex flag that skips all approval prompts)
#   --dangerously-bypass-approvals-and-sandbox  (Codex full sandbox bypass)
#
# Environment:
...
# 3. Attempts to rewrite OACB / Codex config via prompt
#    Includes Codex-specific bypass flags: --yolo and --dangerously-bypass-approvals-and-sandbox
if echo "$prompt" | grep -qiE '(disable[[:space:]]+(oacb[[:space:]]+)?(security|hook|policy|enforcement))|((turn[[:space:]]+off|bypass|skip)[[:space:]]+(oacb|security|hook|policy|enforcement))|--dangerously-skip-permissions|--dangerously-bypass-approvals-and-sandbox|--yolo|disableAutoMode.*false'; then
  emit_block "prompt attempts to disable OACB enforcement or use a Codex permission bypass flag" "OACB-PROMPT-004"
fi
