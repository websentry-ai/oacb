#!/usr/bin/env bash
#
# oacb-post-tool.sh — OACB PostToolUse / PostToolUseFailure hook (Codex CLI agent)
#
# Logs all tool completions and failures to the audit log.
#
# This hook runs async (non-blocking) — exit code is ignored by Codex.
#
# Environment:
...
if [[ "$hook_event" == "PostToolUseFailure" ]] || [[ -n "$error" ]]; then
  jq -cn \
    ...
    '{ts:$ts,decision:"tool-failed",tier:$tier,hook:"PostToolUse",tool:$tool,cmd:$cmd,error:$error,oacb_version:$ver}' \
